...
for m=1:length(NH)
    for n=1:length(DT)
clear AQ1 HAQ1 kp kn Input_ex amoAQ amoh2AQ CurrentsAQ Currentsh2AQ Currents
...
HAQ1=amoh2AQ(:,i);
 ...
 r = -1 + 2.*rand(length(Currents(:,1)),length(Currents(1,:)));

states = [];
for i = 1:step
    a = Currents(:, ML*(i-1)+1:ML*i)+10^(-13)*r(:, ML*(i-1)+1:ML*i);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%jia zao yin
    states(:, i) = a(:);
end
...
sprintf('%s',['f=',num2str(f),'NRMSE:',num2str(NRMSE),'nh=',num2str(NH(m)),'freq=',num2str(freq(n))])
...
xxx=strcat('A',num2str((length(NH)+1)*f-length(NH)));
xlswrite(wenjiamming,nrmse,'sheet1',xxx);
